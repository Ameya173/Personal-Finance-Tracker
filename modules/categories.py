    ...
    def add_category(self):
        ...
        try:
            ...
            try:
                ...
                if hasattr(self, 'on_category_update'):
                    self.on_category_update()
            except Exception as e:
                messagebox.showerror("Database Error", f"Failed to save category: {str(e)}", parent=self.frame)
                # Rollback the changes if save failed
                self.data["categories"].remove(category)
                if category in self.data["category_data"]:
