...
class Database:
    ...
    def load_data(self):
        ...
        try:
            if os.path.exists(DATA_FILE):
                ...
                print("[LOAD] Data file not found, creating new")
                data = {
                    "income": [],
                    "expenses": [],
                    "categories": [],
            ...
            required_keys = ["income", "expenses", "categories", "budget", "goals", "users"]
            for key in required_keys:
                if key not in data:
                    data[key] = [] if key != "budget" else {}
                    print(f"[LOAD] Added missing key: {key}")
    ...
    def save_data(self, data=None):
        ...
        try:
            ...
            temp_file = None  # Successfully renamed, no need to clean up
            print("[SAVE] Financial data saved successfully")
            success = True
        ...
        # Notify callbacks after successful save
        if success:
    ...
    def add_transaction(self, transaction_type, transaction_data):
        ...
        self.data[transaction_type].append(transaction_data)
        success =  self.save_data()
        if success:
            self.notify_callbacks()  # Add this line
        return success
